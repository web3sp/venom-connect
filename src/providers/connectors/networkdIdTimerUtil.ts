...
export const setupNetworkIdTimer = (address: any, provider: any, allowedNetworkIds: number | number[] | undefined) => {
  ...
  if (provider && address && window && !window.venomNetworkIntervalId) {
    window.venomNetworkIntervalId = window.setInterval(async () => {
      ...
      window.updateVenomModal({wrongNetwork: state && address && !isAllowedId})
